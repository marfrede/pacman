...
Particle::Particle(Vector position, Vector velocity, float rotation, float lifetime, float gravity) : TrianglePlaneModel(0.015f, 0.015f, 1, 1) {
...
void Particle::reset(Vector pos) {
	...
	bool particlesOnlyFallDown = true;
	float yDir = rand() % 10 - 5;
	Vector velo = Vector((rand() % 10 - 5), particlesOnlyFallDown? -1 * abs(yDir): yDir, (rand() % 10 - 5));
